...
/** Media3 1.9.0 MP3 extractor with mandatory frame indexing for precise lesson seeks. */
@UnstableApi
public final class JlexaIndexedMp3Extractor implements Extractor {

  /** Factory for {@link JlexaIndexedMp3Extractor} instances. */
  public static final ExtractorsFactory FACTORY = () -> new Extractor[] {new JlexaIndexedMp3Extractor()};
   ...
   *
   * <p>JLexa enables this by default even when header metadata is seekable. Coarse header
   * tables can otherwise assign different time offsets to adjacent segment playbacks.
   * Unlike upstream 1.9.0, this is not only a fallback for unseekable metadata.
   *
   * <p>A cold seek can scan the file prefix to build the time-to-byte index. This is
   * intended for locally stored lesson audio, where accuracy takes priority over reducing reads.
   */
  ...
  private static final String TAG = "JlexaIndexedMp3Extractor";
  ...
  public JlexaIndexedMp3Extractor() {
    this(FLAG_ENABLE_INDEX_SEEKING);
  }

  /**
   * @param flags Flags that control the extractor's behavior.
   */
  public JlexaIndexedMp3Extractor(@Flags int flags) {
   ...
   */
  public JlexaIndexedMp3Extractor(@Flags int flags, long forcedFirstSampleTimestampUs) {
  ...
  private Seeker computeSeeker(ExtractorInput input) throws IOException {
    ...
    // JLexa: even a seekable Xing/VBRI/MLLT map can estimate timestamps
    // differently on adjacent seeks. Build the map from actual frame counts.
    // Stock Media3 1.9.0 only enables this flag for unseekable header maps.
