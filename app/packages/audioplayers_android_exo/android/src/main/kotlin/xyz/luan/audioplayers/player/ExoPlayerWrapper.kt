...
class ExoPlayerWrapper(
    ...
    private var durationSource: FullDurationMediaSource? = null
    override var playbackEndMs: Int? = null
        private set
    ...
    override fun getDuration(): Int? {
        ...
        return durationSource?.durationMs
            ?: if (playbackEndMs == null) {
                (player.duration.takeUnless { it == TIME_UNSET })?.toInt()
            } else {
                null
            }
    }
    ...
    override fun setLooping(looping: Boolean) {
        ...
        player.repeatMode = if (looping && playbackEndMs == null) {
    ...
    override fun setSource(source: Source) {
        ...
        setLooping(requestedLooping)
    }

    @androidx.annotation.OptIn(androidx.media3.common.util.UnstableApi::class)
    override fun setPlaybackEnd(endMs: Int?, positionMs: Int?) {
        if (playbackEndMs == endMs) return
        val currentSource = source ?: error("Set a source before setting its playback end")
        val position = positionMs?.toLong() ?: player.currentPosition
        playbackEndMs = endMs
        // setMediaSource flushes old decoded buffers and preserves playWhenReady.
        // Clipping starts at zero, so both seek and position remain file absolute.
        player.setMediaSource(buildMediaSource(currentSource), position)
        setLooping(requestedLooping)
        player.prepare()
    }
    ...
    private fun buildMediaSource(source: Source): MediaSource {
        ...
        val fullSource = FullDurationMediaSource(rawSource, durationSource?.durationMs)
        durationSource = fullSource
        if (playbackEndMs == null) return fullSource
        return ClippingMediaSource.Builder(fullSource)
            .setStartPositionMs(0)
            .setEndPositionMs(playbackEndMs?.toLong() ?: C.TIME_END_OF_SOURCE)
            .build()
    }
...
/** Observes the child timeline before clipping changes its visible duration. */
@androidx.annotation.OptIn(androidx.media3.common.util.UnstableApi::class)
private class FullDurationMediaSource(
    source: MediaSource,
    initialDurationMs: Int?,
) : WrappingMediaSource(source) {
    // Source callbacks run on the playback thread; channel getters run on main.
    @Volatile var durationMs: Int? = initialDurationMs
        private set

    override fun onChildSourceInfoRefreshed(newTimeline: Timeline) {
        if (!newTimeline.isEmpty) {
            val window = newTimeline.getWindow(0, Timeline.Window())
            if (!window.isLive && window.durationMs != TIME_UNSET) {
                durationMs = window.durationMs.toInt()
            }
        }
        super.onChildSourceInfoRefreshed(newTimeline)
    }
}
